...
#if __has_include(<firebase_core_tvos/FLTFirebasePluginRegistry.h>)
#import <firebase_core_tvos/FLTFirebasePluginRegistry.h>
#else
...
@import CommonCrypto;
// Restored by hand after porting: the porter's compatibility database flags
// AuthenticationServices as tvOS-incompatible because most of its API
// (ASWebAuthenticationSession, SFSafariViewController-adjacent browser
// flows) isn't available on tvOS. But Sign in with Apple
// (ASAuthorizationController / ASAuthorizationAppleIDProvider, used
// extensively below) is part of the same framework and *is* available on
// tvOS 13+ — stripping the import broke a still-supported code path.
#import <AuthenticationServices/AuthenticationServices.h>

#if __has_include(<firebase_core_tvos/FLTFirebaseCorePlugin.h>)
#import <firebase_core_tvos/FLTFirebaseCorePlugin.h>
#else
...
// FIRMultiFactorResolver / FIRMultiFactorAssertion aren't even forward
// `@class`-declared in Firebase's tvOS build of FirebaseAuth (unlike
// FIRMultiFactorSession / FIRTOTPSecret above, which are forward-declared
// but lack a visible interface) — these two ivars don't compile at all on
// tvOS.
#if !TARGET_OS_TV
...
// -[FIRAuth canHandleNotification:] / -canHandleURL: intercept the silent
// push / universal link continuation used by phone-auth verification,
// which isn't exported by Firebase's tvOS Auth pod (see PORTING_REPORT.md
// and the phone-auth disable above). They're simply not reachable on tvOS.
#if !TARGET_OS_TV
...
- (void)authorizationController:(ASAuthorizationController *)controller
           didCompleteWithError:(NSError *)error API_AVAILABLE(macos(10.15), ios(13.0), tvos(13.0)) {
  void (^completion)(InternalUserCredential *_Nullable, FlutterError *_Nullable) =
      self.appleCompletion;
  self.appleCompletion = nil;
  self.appleSignInRequestInFlight = NO;
...
// Multi-factor auth disabled on tvOS by hand (not caught by the porter):
// FIRMultiFactorResolver / FIRMultiFactorAssertion / FIRMultiFactorInfo
// aren't even forward-declared in Firebase's tvOS build of FirebaseAuth
// (build errors were "unknown type name", not just "property not found on
// forward class" as with FIRTOTPSecret) — the whole MFA surface is absent
// from the SDK on this platform, not just TOTP/phone factors specifically.
// See also: getAppMultiFactorFromPigeon, enrollPhoneApp, getEnrolledFactorsApp,
// getSessionApp, unenrollApp, enrollTotpApp, resolveSignInResolverId below,
// the TOTP block above, and the ivars in @implementation's leading braces.
// There are ~9 call sites for this method scattered through sign-in/link/
// reauthenticate flows below; rather than guard every call site, this
// method keeps its one signature and gets a tvOS-only stub body so callers
...
- (void)handleMultiFactorError:(AuthPigeonFirebaseApp *)app
  ...
  completion(nil, [FlutterError
                       errorWithCode:@"second-factor-required"
                             message:@"Multi-factor authentication is not supported by the "
...
- (void)getFIRAuthCredentialFromArguments:(NSDictionary *)arguments
  ...
  // Phone Auth - Only supported on iOS (TARGET_OS_IPHONE also covers tvOS;
  // FIRPhoneAuthProvider isn't exported by Firebase's tvOS Auth pod, see
  // PORTING_REPORT.md).
...
// FIRAuth.APNSToken / -setAPNSToken:type: feed phone-auth silent-push
// verification, which isn't exported by Firebase's tvOS Auth pod (see
// PORTING_REPORT.md).
...
// Multi-factor auth (FIRMultiFactor and everything that touches it below)
// is disabled on tvOS by hand — see the comment above handleMultiFactorError
// for why. The MultiFactorUserHostApi / MultiFactoResolverHostApi /
// MultiFactorTotpHostApi / MultiFactorTotpSecretHostApi Pigeon protocols
// this class declares conformance to (see the @interface) aren't
// statically enforced by Objective-C, and Pigeon's own dispatcher already
// returns a "doesn't respond to selector" FlutterError for any protocol
// method without an implementation — so the methods below are simply
// omitted on tvOS rather than given stub bodies.
#if !TARGET_OS_TV
...
- (nonnull ASPresentationAnchor)presentationAnchorForAuthorizationController:
    (nonnull ASAuthorizationController *)controller API_AVAILABLE(macos(10.15), ios(13.0), tvos(13.0)) {
#if TARGET_OS_OSX
...
#elif TARGET_OS_TV
  completion([FlutterError errorWithCode:@"unsupported-platform"
                                 message:@"Phone/multi-factor authentication is not supported by "
...
#else  // TARGET_OS_TV
// tvOS stubs for the MultiFactorUserHostApi / MultiFactoResolverHostApi
// selectors. MFA is unavailable in the Firebase tvOS SDK, but Pigeon's
// SetUp<HostApi>WithSuffix ASSERTS (`respondsToSelector`) that the plugin
// implements every protocol method at registration time — a missing method
// aborts the app in +registerWithRegistrar: before any Dart runs (this was
// the crash the runtime smoke test caught). So each method must exist and
// simply return an "unsupported on tvOS" FlutterError.
- (void)getEnrolledFactorsApp:(nonnull AuthPigeonFirebaseApp *)app
                   completion:(nonnull void (^)(NSArray<InternalMultiFactorInfo *> *_Nullable,
                                                FlutterError *_Nullable))completion {
  completion(nil, [FlutterError errorWithCode:@"unsupported-platform"
                                      message:@"Multi-factor authentication is not supported "
                                              @"by the Firebase SDK on tvOS."
                                      details:nil]);
}
...
// TOTP multi-factor auth disabled on tvOS by hand (not caught by the
// porter): FIRTOTPSecret is a Swift class (TOTPSecret.swift, "available on
// iOS and macOS" per its own doc comment) that Firebase's CocoaPods
// FirebaseAuth/Auth subspec does not export an Objective-C interface for on
// tvOS — only a forward `@class` declaration is visible, so any property/
// method access on it fails to compile. These five MultiFactorTotpHostApi /
// MultiFactorTotpSecretHostApi methods all funnel through FIRTOTPSecret;
// without an implementation here the Pigeon dispatcher in
// firebase_auth_messages.g.m returns "method not implemented" for them at
// runtime, which is the right behavior until/unless Firebase ships tvOS
// support for TOTP MFA.
#if !TARGET_OS_TV
...
#else  // TARGET_OS_TV
// tvOS stubs for MultiFactorTotpHostApi / MultiFactorTotpSecretHostApi — see
// the MFA stub comment above: Pigeon asserts every selector exists at
// registration, so these must be present and return an unsupported error.
- (void)generateSecretSessionId:(nonnull NSString *)sessionId
                     completion:(nonnull void (^)(InternalTotpSecret *_Nullable,
                                                  FlutterError *_Nullable))completion {
  completion(nil, [FlutterError errorWithCode:@"unsupported-platform"
                                      message:@"TOTP multi-factor auth is not supported by the "
                                              @"Firebase SDK on tvOS."
                                      details:nil]);
}
...
- (void)signInWithProviderApp:(nonnull AuthPigeonFirebaseApp *)app
  ...
  completion(nil, nil);
#elif TARGET_OS_TV
  // -[FIROAuthProvider getCredentialWithUIDelegate:completion:] drives a
  // browser-based OAuth redirect (ASWebAuthenticationSession), which tvOS
  // doesn't support (no WebKit) — every signInWithProvider() call other
  // than Sign in with Apple (handled above via ASAuthorizationController,
  // which doesn't need a browser) needs this on tvOS.
  NSLog(@"signInWithProvider (other than Apple) is not supported on tvOS: it requires a "
        @"browser-based OAuth redirect.");
  completion(nil, [FlutterError errorWithCode:@"unsupported-platform"
                                      message:@"signInWithProvider is only supported for Apple "
                                              @"Sign In on tvOS"
                                      details:nil]);
#else
...
- (void)linkWithProviderApp:(nonnull AuthPigeonFirebaseApp *)app
  ...
  completion(nil, nil);
#elif TARGET_OS_TV
  // See signInWithProviderApp above: browser-based OAuth redirect isn't
  // available on tvOS for any provider other than Apple (handled above).
  NSLog(@"linkWithProvider (other than Apple) is not supported on tvOS: it requires a "
        @"browser-based OAuth redirect.");
  completion(nil, [FlutterError errorWithCode:@"unsupported-platform"
                                      message:@"linkWithProvider is only supported for Apple "
                                              @"Sign In on tvOS"
                                      details:nil]);
#else
...
- (void)reauthenticateWithProviderApp:(nonnull AuthPigeonFirebaseApp *)app
  ...
  // See signInWithProviderApp above: browser-based OAuth redirect isn't
  // available on tvOS for any provider other than Apple (handled above).
  NSLog(@"reauthenticateWithProvider (other than Apple) is not supported on tvOS: it requires "
        @"a browser-based OAuth redirect.");
  completion(nil, [FlutterError errorWithCode:@"unsupported-platform"
                                      message:@"reauthenticateWithProvider is only supported "
                                              @"for Apple Sign In on tvOS"
                                      details:nil]);
#else
  self.authProvider = [FIROAuthProvider providerWithProviderID:signInProvider.providerId];
  NSArray *scopes = signInProvider.scopes;
...
#if TARGET_OS_OSX || TARGET_OS_TV
  // reCAPTCHA app-verification needs WebKit, which tvOS doesn't have.
  NSLog(@"initializeRecaptchaConfigWithCompletion is not supported on this "
        @"platform.");
  completion(nil);
#else
  FIRAuth *auth = [self getFIRAuthFromAppNameFromPigeon:app];
