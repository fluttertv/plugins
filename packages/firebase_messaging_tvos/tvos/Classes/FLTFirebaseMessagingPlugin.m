...
@implementation FLTFirebaseMessagingPlugin {
  ...
  API_AVAILABLE(ios(10), tvos(10), macosx(10.14))
...
- (void)setupNotificationHandlingWithRemoteNotification:(nullable NSDictionary *)remoteNotification
  ...
  if (@available(iOS 10.0, tvOS 10.0, macOS 10.14, *)) {
...
#elif TARGET_OS_TV
  // UIApplicationLaunchOptionsRemoteNotificationKey is unavailable on tvOS
  // (no notification-driven app launch path there).
...
- (void)userNotificationCenter:(UNUserNotificationCenter *)center
    ...
    API_AVAILABLE(macos(10.14), ios(10.0), tvos(10.0)) {
  ...
  // this fix)
  //
  // UNNotificationContent.userInfo (and therefore the FCM payload / message
  // ID it carries) is unavailable on tvOS, so the foreground-message
  // forwarding below (Messaging#onMessage) can't run there — the delegate
  // method itself still fires (used only to control presentation options).
#if !TARGET_OS_TV
  ...
  // (messageID is only defined in the !TARGET_OS_TV block above).
...
// Disabled on tvOS by hand: despite the porter widening this method's
// @available annotation to claim tvos(10.0), the UNUserNotificationCenter
// Delegate protocol itself marks
// userNotificationCenter:didReceiveNotificationResponse:withCompletionHandler:
// (and UNNotificationResponse / -userInfo on its content, used below)
// API_UNAVAILABLE(tvos) in the system headers — tvOS doesn't support
// interactive notification responses (no "tap" gesture model for
// notifications the way iOS/macOS have).
#if !TARGET_OS_TV
- (void)userNotificationCenter:(UNUserNotificationCenter *)center
...
// FlutterFire's messaging plugin hands silent pushes to Firebase Auth's
// phone-verification handler when firebase_auth is also present in the app.
// `-[FIRAuth canHandleNotification:]` is unavailable on tvOS (no phone auth),
// and this whole `__has_include(<FirebaseAuth>)` branch only compiles when
// firebase_auth_tvos is in the same app — so it must be gated on !TARGET_OS_TV
// too, otherwise adding firebase_messaging_tvos + firebase_auth_tvos together
// breaks the build (each compiles fine alone). See PORTING_REPORT.md.
#if __has_include(<FirebaseAuth/FirebaseAuth.h>) && !TARGET_OS_TV
...
// UISceneConnectionOptions.notificationResponse is unavailable on tvOS
// (interactive notification responses don't exist there).
#if !TARGET_OS_TV
...
- (void)messagingGetNotificationSettings:(id)arguments
                    withMethodCallResult:(FLTFirebaseMethodCallResult *)result
    API_AVAILABLE(ios(10), tvos(10), macos(10.14)) {
  UNUserNotificationCenter *center = [UNUserNotificationCenter currentNotificationCenter];
  [center getNotificationSettingsWithCompletionHandler:^(
              UNNotificationSettings *_Nonnull settings) {
...
+ (NSDictionary *)NSDictionaryFromUNNotificationSettings:(UNNotificationSettings *_Nonnull)settings
  ...
  // authorizedStatus — the one UNNotificationSettings property that *is*
  // available on tvOS (push permission grant/deny still applies there).
  ...
  // Every other UNNotificationSettings property below (alertSetting,
  // badgeSetting, soundSetting, carPlaySetting, lockScreenSetting,
  // notificationCenterSetting, timeSensitiveSetting, showPreviewsSetting,
  // criticalAlertSetting, providesAppNotificationSettings) is
  // API_UNAVAILABLE(tvos) in the system headers — tvOS has no
  // banner/sound/badge/lock-screen/notification-center presentation UI to
  // configure, only the underlying push permission grant captured above.
  // All settle to the same "@-1 / unsupported" sentinel already used for
  // pre-iOS-version fallbacks, so tvOS clients see a consistent shape.
#if TARGET_OS_TV
  settingsDictionary[@"announcement"] = @-1;
  settingsDictionary[@"criticalAlert"] = @-1;
  settingsDictionary[@"showPreviews"] = @-1;
  settingsDictionary[@"alert"] = @-1;
  settingsDictionary[@"badge"] = @-1;
  settingsDictionary[@"sound"] = @-1;
  settingsDictionary[@"carPlay"] = @-1;
  settingsDictionary[@"lockScreen"] = @-1;
  settingsDictionary[@"notificationCenter"] = @-1;
  settingsDictionary[@"timeSensitive"] = @-1;
...
// Excluded on tvOS: its only caller (willPresentNotification) is itself
// guarded out there, since UNNotificationContent.userInfo is unavailable
// on tvOS (see willPresentNotification above).
#if !TARGET_OS_TV
#if TARGET_OS_OSX
