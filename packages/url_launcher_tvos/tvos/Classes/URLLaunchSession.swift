...
// SafariServices (SFSafariViewController) does not exist on tvOS, so the entire
// in-app browser session is compiled out. `URLLauncherPlugin` never references
// this type on tvOS — it returns `.noUI` for the in-app path instead.
#if !os(tvOS)
