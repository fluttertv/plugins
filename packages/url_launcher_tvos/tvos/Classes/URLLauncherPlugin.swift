...
public final class URLLauncherPlugin: NSObject, FlutterPlugin, UrlLauncherApi {
  ...
  // tvOS has no SafariServices, so there is never an in-app browser session.
  #if !os(tvOS)
  ...
  func openUrlInSafariViewController(
    ...
    #if os(tvOS)
      // tvOS has no SafariServices / in-app browser. Report that no UI is
      // available; callers should use an external launch mode instead.
  ...
  func closeSafariViewController() {
    // No-op on tvOS: there is never an in-app browser session to close.
    #if !os(tvOS)
