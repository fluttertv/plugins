...
class FlutterSecureStorage {
    ...
    private func parseAccessControlFlags(_ flagString: String?) -> SecAccessControlCreateFlags {
        ...
        for dirtyFlag in flagList {
            ...
            // tvOS has no biometric sensors; `.biometryAny` /
            // `.biometryCurrentSet` are not in the tvOS SDK. Requests
            // for them fall through (the item is still stored, just
            // without biometric gating — the only possible behaviour on
            // a device with no biometrics).
            #if !os(tvOS)
            case "biometryAny":
    ...
    private func baseQuery(from params: KeychainQueryParameters) -> [CFString: Any] {
        ...
        // tvOS has no LocalAuthentication (LAContext); the authentication
        // context is a no-op there. Keychain storage itself is fully
        // supported on tvOS via the Security framework.
        #if !os(tvOS)
